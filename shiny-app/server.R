server <- function(input, output, session) {
  ...
  artists<-read.csv('../data/artists2.csv')
  words<-read.csv('../data/lyrics_words.csv')
  bis<-read.csv('../data/lyrics_bis.csv')
    ...
    reactable(df()%>%select(image_url,artist,track,genre, valence, danceability, acousticness, energy),
            ...
            columns= list(
              ...
              track = colDef(name='TRACK', html=TRUE, cell= function(value,index){
                sprintf('<a style=text-decoration:none;color:#3E6F9A; href="%s" target="_blank">%s</a>', df()$song_url[index], value)
              }),
  ...
  output$plotlyScatter <- renderPlotly({
    p<-ggplot(df())+
      geom_point(aes(x=get(input$xScatter),
                     ...
                     fill=genre), alpha = 0.75, stroke=0.25, size=3, shape=21, color='white')+
